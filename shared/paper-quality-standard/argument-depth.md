...
- **Normative Statement:** 主模型说明应从题目特征说明为何采用当前模型关系，并覆盖变量定义依据、关系建立依据、目标函数来源、关键约束的现实含义、参数来源、求解方法、输出与题目之间的对应关系，以及验证接口；关键公式还应交代其在当前问题中的作用和符号语义，关键参数应能追溯到数值、单位、来源或推导依据；主要模型、算法或处理模块还应说明其责任、输入输出、前后接口以及移除后会失去的能力。
- **Scope:** 模型建立与求解章节。
...
- **Normative Statement:** 核心结果讨论应包含主要现象、定量证据、模型或机理解释以及对题目子问题的意义，不能只复述图表趋势；存在自然对照时应交代比较对象和差异，没有自然对照时不应强行制造基线；主要结果还应落到题目要求的输出、判断或决策含义上。
