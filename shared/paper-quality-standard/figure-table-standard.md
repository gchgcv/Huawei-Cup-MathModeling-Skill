...
- **Normative Statement:** 正式图表应展示正文难以同等清晰表达的趋势、结构、分布、空间关系、网络、流程分支或多变量比较，并承担一个可识别的主要论证职责；纯文字装框、线性词语箭头链和装饰性总览不构成有效图形。
- **Scope:** 正式论文图表。
...
- **Normative Statement:** 核心图表后的讨论应给出关键数值、模型或机理解释以及与题目子问题的关系，不能停留在“由图可知”或趋势复述；复杂图表还应提供必要的读图顺序、重点区域或子图关系，使读者能够识别主要信息。
- **Scope:** 核心结果图表。
...
- **Normative Statement:** 中文论文中的主要坐标轴名称、图例、方案或类别名称、注释和说明文字原则上使用中文；模型名、算法名、变量、单位、通用缩写及翻译可能产生歧义的专业术语可以保留标准形式。
- **Scope:** 中文论文中的正式图表及其图内文字。
- **Required Exceptions:** 用户模板、题目要求或外部引用对象明确要求保留原文时，可以使用原文，但应与正文语境保持一致。
- **Relations:** `TERM-001`, `TERM-002`, `FMT-004`。
